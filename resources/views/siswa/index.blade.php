...
<body>
    ...
    <div class="container">
        <h2 style="text-align: center; color: #6d4c41;">Data Siswa</h2>
        @if(session('success'))
...
@endif
        <div class="search-container">
            ...
            <form action="{{ route('siswa.index') }}" method="GET" style="display: flex; gap: 10px;">
                <input type="text" name="search" placeholder="Cari siswa..." value="{{ request('search') }}">
                <button type="submit" class="btn">🔍 Cari</button>
                <a href="{{ route('siswa.print', ['query' => $query ?? '']) }}" class="btn">🖨️ Print</a>
            </form>
        </div>
                ...
                <tbody>
                    @foreach ($siswa as $item)
                        <tr>
                            ...
                            <td>{{ $item->jurusan }}</td>
                            <td>
                                <a href="{{ route('siswa.edit', $item->nisn) }}" class="btn">✏️ Edit</a>
